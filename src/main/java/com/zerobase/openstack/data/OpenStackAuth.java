...
public class OpenStackAuth {
    ...
    public static OpenStackAuth projectScopedAuth(String username, String password, String domain, String projectName)
            throws NullPointerException,
            ClientResponseException,
            AuthenticationException {
        return new OpenStackAuth(OSFactory.builderV3()
                .endpoint(Constants.OPENSTACK_KEYSTONE_URL + Constants.KEYSTONE_API_VERSION)
                .credentials(username,password, Identifier.byName(domain))
                .scopeToProject(Identifier.byName(projectName), Identifier.byName(domain))
                .authenticate());
    }
